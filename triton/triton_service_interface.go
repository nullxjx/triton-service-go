...
func (t *TritonClientService) makeHTTPPostRequestWithHeaders(
	uri string, reqBody []byte, timeout time.Duration, headers map[string]string,
) (*fasthttp.Response, error) {
	requestObj := t.acquireHTTPRequest(fasthttp.MethodPost)
	for key, value := range headers {
		requestObj.Header.Set(key, value)
	}
	responseObj := fasthttp.AcquireResponse()
	defer fasthttp.ReleaseRequest(requestObj)
...
func (t *TritonClientService) httpErrorHandler(statusCode int, httpErr error) error {
	if httpErr == nil {
		return errors.New("[HTTP]code: " + strconv.Itoa(statusCode) + "; error: http infer failed")
	}
...
func (t *TritonClientService) ModelHTTPInferWithHeaders(
	requestBody []byte, headers map[string]string,
	modelName, modelVersion string,
	timeout time.Duration,
	decoderFunc DecoderFunc,
	params ...interface{},
	...
	modelInferResponse, inferErr := t.makeHTTPPostRequestWithHeaders(
		t.getServerURL()+TritonAPIForModelPrefix+modelName+TritonAPIForModelVersionPrefix+modelVersion+"/infer",
		requestBody,
		timeout,
		headers,
...
func (t *TritonClientService) ModelGRPCStreamInfer(
	id string,
	tensors []*ModelInferRequest_InferInputTensor,
	rawInput [][]byte,
	inferParams map[string]*InferParameter,
	modelName, modelVersion string,
	timeout time.Duration,
	decoderFunc DecoderFunc,
	params ...interface{},
	...
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	var response *ModelStreamInferResponse
	...
	streamCli, err := t.grpcClient.ModelStreamInfer(ctx)
	if err != nil {
		return nil, errors.New("grpc stream infer error")
	}
	start := time.Now().UnixMilli()
	go func() {
		err := streamCli.Send(&request)
		if err != nil {
			return
		}
	...
	for {
		response, err = streamCli.Recv()
		if err == io.EOF {
			log.Warnf("ModelInferRequest modelStreamInferClient.Recv io EOF")
			// The server closed the stream
			break
		}
		if err != nil {
			return nil, errors.New(fmt.Sprintf("modelStreamInferClient.Recv error: %v", err))
		}
		if response.InferResponse != nil {
			break
		}
	}
	end := time.Now().UnixMilli()
	...
	rsp, decodeErr := decoderFunc(response.InferResponse, params...)
	if decodeErr != nil {
		return nil, t.decodeFuncErrorHandler(decodeErr, true)
	}
	return rsp, nil
